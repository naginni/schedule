class Mutation
		...
		if @length >= cromosome.size
			...
			size = (cromosome.size - 1)
				...
				(0..4).each do |x| 
					...
					# tengo los genes que se van a mutar
					random1 = rand(cromosome.size)
					gene1 = cromosome.getGene(random1)
					# tengo los genes que se van a mutar 
					random2 = rand(cromosome.size)
					gene2 = cromosome.getGene(random2)
					# realizo la mutacon
					cromosome.setGene(random2,gene1)
					cromosome.setGene(random1,gene2) 
			end 
   end
   ...
   # se realiza la busquedad secuencial
	def self.busquedadSecuencial(array)
